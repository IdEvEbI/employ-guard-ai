...
| 项         | 口径                                                                                                           |
| ---------- | -------------------------------------------------------------------------------------------------------------- |
| 阶段       | **投前看简历 · 排查增强**（#86 已合入；稳定性修补）                                                            |
| 当前 Issue | [#88](https://github.com/IdEvEbI/employ-guard-ai/issues/88) `fix(parse): 规范化文末截断或丢失章节则回退规则层` |
| 做完的标志 | 半句截断 / 抽出已有「教育经历」等章节丢失的 LLM norm 被拒绝并回退规则层；parse 标明降级                        |
| 现在不做   | 项目审阅 JSON 非法回退（另开）；发给模型前压图；听后做复盘；GUI / Lang\*；R16；`resume` 直接吃 `.docx`         |
...
| **投前看简历 · 排查增强**       | 强制重跑、排查模式、并行、批跑总表、安装说明、OCR 等（仍专注简历）     | 已完成（R7～R13 / R3b / #81 / #83 / #86）；稳定性见 **#88** |
...
顺序：R17 →…→ R26。R16 不插队。R13 / #81 / #83 / #86 已合入；当前尖端 [#88](https://github.com/IdEvEbI/employ-guard-ai/issues/88)（规范化文末截断 / 丢章节回退）。
