...
| 项         | 口径                                                                                                    |
| ---------- | ------------------------------------------------------------------------------------------------------- |
| 阶段       | **投前看简历 · 排查增强**（R13 / #81 已合入；稳定性修补）                                               |
| 当前 Issue | [#83](https://github.com/IdEvEbI/employ-guard-ai/issues/83) `fix(parse): 规范化坏正文回退抽出 / 规则层` |
| 做完的标志 | 思考过程 / 占位 / 过短的 norm 被拒绝并回退规则层；parse 标明降级；后续步不读脏正文                      |
...
| **投前看简历 · 排查增强**       | 强制重跑、排查模式、并行、批跑总表、安装说明、OCR 等（仍专注简历）     | 已完成（R7～R13 / R3b / #81）；稳定性见 **#83**  |
...
顺序：R17 →…→ R26。R16 不插队。R13 / #81 已合入；当前尖端 [#83](https://github.com/IdEvEbI/employ-guard-ai/issues/83)（规范化坏正文回退）。
