...
| 项         | 口径                                                                                                      |
| ---------- | --------------------------------------------------------------------------------------------------------- |
| 阶段       | **投前看简历 · 排查增强**（#83 已合入；稳定性修补）                                                       |
| 当前 Issue | [#86](https://github.com/IdEvEbI/employ-guard-ai/issues/86) `fix(layout): 看图超时加长并对齐、超时可重试` |
| 做完的标志 | 看图默认超时 ≥180 秒且可覆盖；超时 / 断连重试后仍失败才硬失败；不得编造 P2～P5                            |
| 现在不做   | 发给模型前压图；项目审阅 JSON 非法回退（另开）；听后做复盘；GUI / Lang\*；R16；`resume` 直接吃 `.docx`    |
...
| **投前看简历 · 排查增强**       | 强制重跑、排查模式、并行、批跑总表、安装说明、OCR 等（仍专注简历）     | 已完成（R7～R13 / R3b / #81 / #83）；稳定性见 **#86** |
...
顺序：R17 →…→ R26。R16 不插队。R13 / #81 / #83 已合入；当前尖端 [#86](https://github.com/IdEvEbI/employ-guard-ai/issues/86)（看图超时加长并重试）。
