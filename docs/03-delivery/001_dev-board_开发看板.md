...
| 项         | 口径                                                                                                                   |
| ---------- | ---------------------------------------------------------------------------------------------------------------------- |
| 阶段       | **投前看简历 · 排查增强**（#88 已合入；稳定性修补）                                                                    |
| 当前 Issue | [#90](https://github.com/IdEvEbI/employ-guard-ai/issues/90) `fix(projects): 项目审阅 JSON 非法时回退，勿硬中断 resume` |
| 做完的标志 | JSON 非法 / 思考过程时写出降级报告、不编造档次；`resume` 不因本步硬失败；judge 不因此写成不能投                        |
| 现在不做   | 发给模型前压图（#91）；听后做复盘；GUI / Lang\*；R16；`resume` 直接吃 `.docx`                                          |
...
| **投前看简历 · 排查增强**       | 强制重跑、排查模式、并行、批跑总表、安装说明、OCR 等（仍专注简历）     | 已完成（R7～R13 / R3b / #81 / #83 / #86 / #88）；稳定性见 **#90** |
...
「判能不能投」和「查排版」必须先有书面合格线。检查对象仍是 PDF；缺 PDF 时先用 R13 `word-to-pdf` 转出，再跑 `resume`。
...
顺序：R17 →…→ R26。R16 不插队。R13 / #81 / #83 / #86 / #88 已合入；当前尖端 [#90](https://github.com/IdEvEbI/employ-guard-ai/issues/90)（项目审阅 JSON 非法降级）。
...
| v0.2.40 | 2026-09-08 | #81 / PR #84 已合入；开 #83（规范化思考过程 / 过短回退规则层）         |
| v0.2.41 | 2026-09-09 | #83 / PR #85 已合入；开 #86（看图超时加长并对齐文本 LLM、超时可重试）  |
| v0.2.42 | 2026-09-09 | #86 / PR #87 已合入；开 #88（规范化文末截断 / 丢失章节回退规则层）     |
| v0.2.43 | 2026-09-09 | #88 / PR #89 已合入；开 #90（项目审阅 JSON 非法降级，勿硬中断 resume） |
