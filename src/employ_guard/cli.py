...
@app.command("check-skills")
def check_skills_cmd(
    source: Path = typer.Argument(
        ...,
        help="PDF（须已 read-resume）或 *.resume.md / 文本文件。",
    ),
) -> None:
    """检查技能分组、与意向同向、与项目覆盖。本步不替代整份判能不能投。"""
    try:
        result = check_skills(source)
    except CheckSkillsError as exc:
        typer.secho(str(exc), err=True, fg=typer.colors.RED)
        raise typer.Exit(code=1) from exc

    typer.echo(f"已写出技能结构检测：{result.report_md}")
    typer.echo("本步只做技能分项，不替代整份判能不能投，不评排版。")
    status_zh = {"pass": "过本项", "fail": "未过本项", "doubtful": "存疑"}.get(
        result.status, result.status
    )
    color = {
    ...
    typer.secho(f"本项结论：{status_zh}（{result.status}）", fg=color)
    typer.echo(f"岗位意向：{result.target_role or '（未抽到 / 首页未见）'}")
