...
当前里程碑是 **投前看简历**：书面标准见 [04-standard](./docs/04-standard/README.md)；出图与抽文本已可用。产品范围以 [产品说明](./docs/01-product/001_prd_就业守护助手产品说明.md) 为准。当前推进 **查排版**（[#17](https://github.com/IdEvEbI/employ-guard-ai/issues/17)）。
...
`pdf-to-images` 把投递用 PDF 按页写成 `data/output/.../pages/`（本步不评价排版）。`read-resume` 抽出文本（去掉版面空格；页码只在 JSON；本步不判断能不能投）。真实简历放在 `data/input/`，不要提交。密钥放在 `.env`（从 `.env.example` 复制），不要提交。
