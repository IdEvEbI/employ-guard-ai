...
当前里程碑是 **投前看简历 · 结构化链式检查**：R17～R22 已合入；听后做复盘暂缓；**R13 本轮跳过**。当前推进 **R23**（`resume` 编排结构化链：挂入 parse / profile / skills / [#70](https://github.com/IdEvEbI/employ-guard-ai/issues/70)）。顺序见 [开发看板](./docs/03-delivery/001_dev-board_开发看板.md) §5.2。

`parse-resume` / `check-profile` / `check-skills` / `review-projects` 均可单独跑；完整 `resume` 默认挂入（排查关掉贵步骤，仍跑 parse 与 profile）。命令列表以 `uv run employ-guard --help` 与产品说明第 4 节为准。
